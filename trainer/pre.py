...
class PreTrainer(object):
    ...
    def save_model(self, name):
        ...
        # 注意，这里保存了encoder的信息(不带"encoder."前缀)，因此在meta中载入pretrain的参数时，每个参数的key前面要加上"encoder."
    ...
    def train(self):
        ...
        writer.close()
    def eval(self):
        """The function for the pre-eval phase."""
        # Load the logs
        ...
        if self.args.eval_weights is not None:
            pretrained_dict=torch.load(self.args.eval_weights)['params']
            pretrained_dict = {'encoder.'+k: v for k, v in pretrained_dict.items()} #改个名，前面加上一个"encoder."，这样才能载入参数
            self.model.load_state_dict(pretrained_dict)
        else:
            pretrained_dict=torch.load(osp.join(self.args.save_path, 'max_acc' + '.pth'))['params']
            pretrained_dict = {'encoder.'+k: v for k, v in pretrained_dict.items()} #改个名，前面加上一个"encoder."，这样才能载入参数
