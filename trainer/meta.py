...
class MetaTrainer(object):
    ...
    def __init__(self, args):
        ...
        save_path1 = "(use_ResNet50_without_freeze_and_do_pretrain_myself)_" + save_path1 # !!! 修改存储路径
        ...
        self.model_dict = self.model.state_dict() #读取meta的参数列表
        if self.args.init_weights is not None: #如果有手动传入pretrain的参数，则用手动传入的
            pretrained_dict = torch.load(self.args.init_weights)['params']
        else: #不然去logs/pre下寻找对应的训练参数文件夹下的max_acc.pth，从中load pretrain的参数
            ...
            pre_save_path1 = "(use_ResNet50_without_freeze_and_do_pretrain_myself)_" + pre_save_path1 # !!! 修改存储路径
        ...
        # 注意，pretrain中save_model中只保存了encoder内的信息(不带"encoder."前缀)，因此在这里中载入pretrain的参数时，每个参数的key前面要加上"encoder."
        pretrained_dict = {'encoder.'+k: v for k, v in pretrained_dict.items()} #改个名，前面加上一个"encoder."，这样才能载入参数
        pretrained_dict = {k: v for k, v in pretrained_dict.items() if k in self.model_dict} #将pretrained_dict里不属于meta的model_dict的key剔除掉(但实际上原代码中pretrain保存的中没有不属于meta的model_dict的key)
        print(pretrained_dict.keys()) #输出剩下要载入的参数的key
        self.model_dict.update(pretrained_dict) #更新参数到一个临时变量model_dict中
        self.model.load_state_dict(self.model_dict) #把参数实际加载进model里
    ...
    def train(self):
        ...
        for epoch in range(1, self.args.max_epoch + 1):
            ...
            # tqdm库可以显示进度条将处理情况进行可视化展示,tqdm.tqdm()可生成一个tqdm迭代器供enumerate使用
            # 传入的第一个参数应该是一个可迭代的类型(这里传入的是一个class，通过在它内部定义__getitem__()可以使clas变成可迭代的类型)
            tqdm_gen = tqdm.tqdm(self.train_loader)
            # for index, value in enumerate(s, firstindex): (即依次枚举s中的元素，返回下标index和对应的value,firstindex表示第一个value对应的下标从firstindex开始)
            for i, batch in enumerate(tqdm_gen, 1):
                ...
                tqdm_gen.set_description('Epoch {}, Loss={:.4f} Acc={:.4f}'.format(epoch, loss.item(), acc)) # 实时查看每次处理的数据
